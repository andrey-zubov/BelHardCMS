...
from recruit.models import Recruiter, RecruitEducation, RecruitCertificate
...
class RecruiterEditEducationTests(TestCase):
    ...
    def test_GET_user(self):
        ...
        edu = RecruitEducation.objects.create(recruit_edu=self.client_inst,
                                              ...
                                              date_end=self.TEST_DATA_1['date_end'] if self.TEST_DATA_1[
                                                  'date_end'] else None,
                                              )
        certificate = RecruitCertificate.objects.create(education=edu,
                                                        link=self.TEST_DATA_1['certificate'][0],
                                                        img=self.TEST_DATA_1['certificate'][1],
                                                        )
        edu_arr = [{'id': edu.id,
                    'recruit_edu_id': self.client_inst.id,
                    'institution': self.TEST_DATA_1['institution'],
                    'subject_area': self.TEST_DATA_1['subject_area'],
                    'specialization': self.TEST_DATA_1['specialization'],
                    'qualification': self.TEST_DATA_1['qualification'],
        ...
        self.assertEqual(response.context['data']['rec_edu'], edu_arr)
    ...
    def test_POST_user(self):
        ...
        user_edu = RecruitEducation.objects.get(recruit_edu=self.client_inst)
        user_cert = RecruitCertificate.objects.get(education=user_edu.id)
